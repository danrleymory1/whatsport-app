// src/app/components/manager/reservations.tsx
"use client";
...
export function ManagerReservations() {
  ...
  const fetchSpaces = async () => {
    try {
      ...
      if (response.data && response.data.spaces) {
        ...
        if (response.data.spaces.length > 0 && !selectedSpace) {
          setSelectedSpace(response.data.spaces[0].id);
        }
      }
    } catch (error) {
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      const response = await apiService.getSpaceReservations(selectedSpace, filters);
  ...
  const handleRejectReservation = async () => {
    ...
    try {
      await apiService.rejectReservation(selectedReservation.id, { rejection_reason: rejectionReason });
  ...
  const handleCompleteReservation = async (reservationId: string) => {
    try {
      await apiService.completeReservation(reservationId);
